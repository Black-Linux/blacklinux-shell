...
version="1.0-Beta"
#uptodate="All packages are up to date. Nothing to upgrade at the moment!"
...
if [[ $1 = "add" ]]; then
    if [ "$EUID" -ne 0 ]
        then
            echo -e "${red}ERROR:${reset} You're not an root! Use sudo xad <command>"
        exit
    fi
...
if [[ $1 = "version" ]]; then
    ...
    echo -e "Patter is available on Blacklinux, and Debian based-systems using version above 9."
